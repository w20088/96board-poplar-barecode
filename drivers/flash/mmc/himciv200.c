			   ...
			   struct mmc_data *data)
{
	...
	HIMCI_DEBUG_INFO("Send cmd of card is CMD %d", cmd->cmd_index);
...
static void hi_mci_set_ios(struct mmc_dev_t *mmc_dev)
{
	...
	if (mmc_dev->card_caps & MMC_MODE_DDR_52MHz) {
		/* eMMC clock phase */
		tmp_reg = readl(REG_BASE_CRG + REG_EMMC_PERI_CRG);
		tmp_reg &= ~SDIO1_SAP_PS_SEL_MASK;
		tmp_reg |= SDIO1_SAP_PS_SEL_225;
		writel(tmp_reg, REG_BASE_CRG + REG_EMMC_PERI_CRG);
	}
#endif
}
...
#if defined(CONFIG_ARCH_HI3798MX) || defined(CONFIG_ARCH_HIFONE) || defined(CONFIG_ARCH_HI3798CV2X)
